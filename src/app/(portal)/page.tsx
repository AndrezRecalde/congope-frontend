...
import { PortalFiltros } from "@/components/portal/PortalFiltros";
import { PortalEstadisticas } from "@/components/portal/PortalEstadisticas";
import { PortalEmblematicoSection } from "@/components/portal/PortalEmblematicos";
import { PortalBuenasPracticas } from "@/components/portal/PortalBuenasPracticas";
import { ProyectoDrawerPortal } from "@/components/portal/ProyectoDrawerPortal";
...
const PortalMapa = dynamic(
  () => import("@/components/portal/PortalMapa").then((m) => m.PortalMapa),
  { ssr: false },
);
...
export default function PortalPage() {
  ...
  const [catalogos, setCatalogos] = useState<CatalogosMapa | null>(null);
  const [conteos, setConteos] = useState<ConteosPortal | null>(null);
  const [estadisticas, setEstadisticas] = useState<EstadisticasPortal | null>(
    null,
  );
  ...
  // Tooltip del mapa
  const [tooltipProv, setTooltipProv] = useState<ProvinciaPortal | null>(null);
  ...
  const handleHoverProvincia = useCallback(
    (datos: ProvinciaPortal | null, pos: { x: number; y: number } | null) => {
    ...
    >
      ...
      >
        ...
        {tooltipProv && tooltipPos && (
          <div
            ...
            style={{
              left:
                tooltipPos.x > window.innerWidth - 240
                  ? tooltipPos.x - 220
                  : tooltipPos.x + 16,
              top:
                tooltipPos.y > window.innerHeight - 200
                  ? tooltipPos.y - 180
                  : tooltipPos.y + 16,
            }}
          >
      ...
      {estadisticas && (
        <div className="scroll-reveal">
          <PortalEstadisticas estadisticas={estadisticas} />
        </div>
      )}
      ...
      <div className="scroll-reveal">
        <PortalEmblematicoSection
          ...
          emblematicos={resultados?.emblematicos ?? []}
          hayFiltros={hayFiltros}
          cargando={cargandoFiltros}
        />
      </div>
      ...
      <div className="scroll-reveal">
        <PortalBuenasPracticas
          ...
          practicas={resultados?.buenas_practicas ?? []}
          hayFiltros={hayFiltros}
          cargando={cargandoFiltros}
        />
      </div>
