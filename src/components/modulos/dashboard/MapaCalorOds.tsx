...
import {
  ...
  Tooltip, Button, Drawer, ScrollArea,
  ThemeIcon, Divider, ActionIcon,
} from '@mantine/core';
...
// Interpolación de blanco a azul marino
const COLOR_ESCALA = [
      ...
      <Stack gap="lg" p="md">
        ...
        <Stack gap="sm">
          ...
          {total === 0 && (
            ...
            >
              <Text size="xs">
                No hay proyectos que combinen este
                ODS con esta provincia. Esta
                combinación podría ser una
                oportunidad para futuras iniciativas.
              </Text>
            </Alert>
          )}
        </Stack>
...
export function MapaCalorOds() {
  ...
  // null = mostrar todos
  const [odsSeleccionados, setOdsSeleccionados] = useState<number[] | null>(null);
  ...
  const option = useMemo(() => {
    ...
    const odsFiltrados = odsSeleccionados
      ? ods.filter((o) =>
          odsSeleccionados.includes(o.id)
        )
      : ods;
    ...
    matriz.forEach((c) => {
      mapaTotal.set(
        `${c.ods_id}:${c.provincia_id}`, c.total
      );
    });
    ...
    return {
      ...
      visualMap: {
        ...
        bottom:    '2%',
        inRange: {
          color: COLOR_ESCALA.map((c) => c[1] as string),
        },
  ...
  const handleClick = useCallback(
    (params: {
      ...
      const [xIdx, yIdx, total] = params.data;
      const prov = (Array.isArray(data.provincias) ? data.provincias : [])[xIdx];
      const odsArray = Array.isArray(data.ods) ? data.ods : [];
      const itemsFiltrados = odsSeleccionados
        ? odsArray.filter(
            (o) => odsSeleccionados.includes(o.id)
          )
        : odsArray;
  ...
  const itemsVisibles = odsSeleccionados
    ? odsArrayVisible.filter(
        (o) => odsSeleccionados.includes(o.id)
      )
    : odsArrayVisible;

  const alturaGrafico = Math.max(
    itemsVisibles.length * 30 + 180, 300
  );
  ...
  if (isError || !data) {
    return (
      <Alert
        ...
        radius="md"
      >
        No se pudo obtener los datos. Intenta
        recargar la página.
      </Alert>
    );
  }
  ...
  if (!data.ods || !Array.isArray(data.ods) || data.ods.length === 0 || data.max_valor === 0) {
    return (
      <Paper p="xl" radius="xl"
        style={{
          border:
      ...
      >
        <Stack align="center" gap="md" py="xl">
          ...
          <Text size="sm" c="dimmed" ta="center"
            maw={360}>
            No hay proyectos con ODS y provincias
            asignados. El mapa aparecerá cuando se
            registren proyectos con esta información.
          </Text>
        </Stack>
      </Paper>
    );
      ...
      >
        <Stack gap="lg">
          ...
          <Group justify="space-between"
            align="flex-start" wrap="wrap" gap="sm">
            <div>
              <Group gap="xs" mb={4}>
                <Title order={4}>
                ...
                <Badge variant="light" color="blue">
                  {data.ods?.length || 0} ODS ×{' '}
                  {data.provincias?.length || 0} provincias
                </Badge>
              </Group>
              <Text size="sm" c="dimmed">
                Proyectos por Objetivo de Desarrollo
                Sostenible y provincia. Haz clic en
                cualquier celda para ver el detalle.
              </Text>
            </div>
          ...
          >
            <Group gap={6} wrap="nowrap" pb={4}>
              {(Array.isArray(data.ods) ? data.ods : []).map((odsItem) => {
                const activo =
                  !odsSeleccionados ||
                  odsSeleccionados.includes(odsItem.id);

                return (
                  <Tooltip
                    key={odsItem.id}
                    label={`ODS ${odsItem.numero} · ${
                      odsItem.nombre
                    }`}
                    position="bottom"
                    withArrow
                    ...
                    <div
                      onClick={() => {
                        if (!odsSeleccionados) {
                          ...
                          odsSeleccionados.includes(
                            odsItem.id
                          )
                        ) {
                          // Quitar del filtro
                          const nuevo =
                            odsSeleccionados.filter(
                              (id) => id !== odsItem.id
                            );
                          setOdsSeleccionados(
                            nuevo.length === 0
              ...
              {odsSeleccionados && (
                ...
                >
                  · {odsSeleccionados.length} de{' '}
                  {data.ods?.length || 0} ODS visibles
                </Text>
              )}
            </Group>
          </ScrollArea>
          ...
          <Text size="xs" c="dimmed" ta="center">
            Haz clic en los números de ODS de arriba
            para filtrar filas. Clic en una celda
            para ver el detalle.
          </Text>
