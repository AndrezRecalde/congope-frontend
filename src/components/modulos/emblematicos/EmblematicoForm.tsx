...
import {
  ...
  Button, Group, Box, LoadingOverlay,
} from '@mantine/core';
...
import type {
  EmblematicoFormValues,
} from '@/types/emblematico.types';
import type { CreateEmblematicoDto } from
...
export function EmblematicoForm({
  ...
  const opcionesProyectos = (proyectosData?.data ?? []).map(
    (p) => ({
      value: p.id,
  ...
  const opcionesProvincias = (provinciasData ?? []).map(
    (p) => ({
      value: p.id,
      label: p.nombre,
    })
